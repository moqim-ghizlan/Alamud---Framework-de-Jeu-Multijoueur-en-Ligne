...
from mud.actions import (
    ...
    PushAction, TeleportAction, EnterAction, LeaveAction, PutAction, UseAction, DoAction,
)
...
def make_rules():
    ...
    return (
	    ...
	    (PutAction      , r"mettre %s(\S+)" % DETS),
        (UseAction      , r"utiliser %s(\S+)" % DETS),
        (DoAction       , r"faire %s(\S+)" % DETS),
    )
